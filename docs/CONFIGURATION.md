...
Environment variables use the `SPELUNK__` prefix with double underscores for hierarchy:

```bash
# Set allowed paths
export SPELUNK__AllowedPaths__0=/path/to/project1
export SPELUNK__AllowedPaths__1=/path/to/project2

# Set initial workspace
export SPELUNK__InitialWorkspace=/path/to/workspace.sln

# Set logging level
export SPELUNK__Logging__MinimumLevel=Debug

# Set server options
export SPELUNK__Server__RequestTimeoutSeconds=300
export SPELUNK__Server__MaxWorkspaces=20
```
...
- `SPELUNK_ALLOWED_PATHS` - Colon-separated list of allowed paths
- `SPELUNK_WORKSPACE` - Initial workspace path
...
Configuration files (`appsettings.json`, `spelunk.config.json`) support hot reload. When changes are detected:
