...
2. **Preview Changes**: Most modification tools show previews - review them
3. **Use Markers for Complex Operations**: Track statements through multiple transforms
4. **Combine Tools**: Use discovery tools to find targets, then modification tools to change them
5. **Understand Scope**: Some tools work on single files, others across the workspace
6. **Pattern Syntax**: Use regex patterns for flexible matching in find/edit operations
...
- ✅ Workspace: load-workspace, workspace-status
